...
describe('authRouter', () =>
{
  describe('postRoute', () => {
    ...
    it('is BadRequest if request invalid', sinon.test( function()
    {
      ...
      userServiceMock.expects('createToken').once().yields( { statusCode : 400, payload : 'Completely invalid request'})
    ...
    it('is InternalServerError if request fails', sinon.test( function()
    {
      ...
      userServiceMock.expects('createToken').once().yields( { statusCode : 500, payload : 'A very important error occurred'})
    ...
    it('is NotFound if request username is not found', sinon.test( function()
    {
      ...
      userServiceMock.expects('createToken').once().yields( { statusCode : 404, payload : 'Username not found'})
    ...
    it('is Ok if request is valid', sinon.test( function()
    {
      ...
      userServiceMock.expects('createToken').once().yields( { statusCode : 200, payload : 'this_is_a_token'})
